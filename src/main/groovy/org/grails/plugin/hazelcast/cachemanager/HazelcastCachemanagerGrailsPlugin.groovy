...
class HazelcastCachemanagerGrailsPlugin extends Plugin {
    ...
    def title = "Hazelcast Cachemanager" // Headline display name of the plugin
...
//    def scm = [ url: "http://svn.codehaus.org/grails-plugins/" ]
    ...
    Closure doWithSpring() { {->
        ...
        def config = grailsApplication.config.grails.hazelcast.cache

        if (config.hazelcastBean) {

            grailsCacheManager(HazelcastGrailsCacheManager) {
                hazelcastInstance = ref(config.hazelcastBean)
            }

            grailsCacheConfigLoader(HazelcastConfigLoader) {
